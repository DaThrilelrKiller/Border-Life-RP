...
_weps = weapons AR_PlayerVaribale;
_mags = magazines AR_PlayerVaribale;

_holder = createVehicle ["weaponholder", getPosATL AR_PlayerVaribale, [], 0, "CAN_COLLIDE" ];
{
...
removeAllWeapons AR_PlayerVaribale;

_array = AR_PlayerVaribale getVariable ["dtk_storage",[[],[]]];
_items = _array select 0;
_amounts = _array select 1;

{
	if (_x call INV_getitemDropable)then
	{
		_amount = _amounts select _ForEachIndex;
		_class = if(_x == "geld")then{"EvMoney"}else{"Suitcase"};
		_object = _class createvehicle position AR_PlayerVaribale;
